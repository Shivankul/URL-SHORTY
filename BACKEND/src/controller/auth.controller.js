...
export const login_user = wrapAsync(async (req, res) => {
    ...
    const {token,user} = await loginUser(email, password); // Assuming loginUser is modified to handle login as well
    req.user = user; // Assuming loginUser returns the user object
    res.cookie('accesstoken', token, cookieOptions); // Set the token in a cookie
    res.status(200).json({user:user, message: "User logged in successfully" }); // Sending the token as a response
});